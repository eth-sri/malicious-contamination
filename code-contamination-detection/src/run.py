...
def load_model(name1, name2):
    if "davinci" in name1:
        ...
        model1, tokenizer1 = contamination.load_model(name1, return_tokenizer=True) # NOTE: changed this
        model1.eval()
        # tokenizer1 = AutoTokenizer.from_pretrained(name1) # NOTE: changed this

    if "davinci" in name2:
        model2 = None
        tokenizer2 = None
    else:
        model2, tokenizer2 = contamination.load_model(name2, return_tokenizer=True) # NOTE: changed this
        model2.eval()
        # tokenizer2 = AutoTokenizer.from_pretrained(name2) # NOTE: changed this
...
def evaluate_data(test_data, model1, model2, tokenizer1, tokenizer2, col_name, modelname1, modelname2):
    ...
    # test_data = test_data[:100] # NOTE: skipped this
...
if __name__ == '__main__':
    ...
    # args.output_dir = f"{args.output_dir}/{args.target_model}_{args.ref_model}/{args.key_name}" # NOTE: skipped this
    Path(args.output_dir).mkdir(parents=True, exist_ok=True)

    source = None
    if args.was_trained_source is not None: # NOTE: added this
        source = pd.read_csv(args.was_trained_source)
        source = source[source['was_trained'] == True]
    ...
    if "jsonl" in args.data:
        ...
        if source is not None:
            dataset = dataset.filter(lambda x: x['question'] in list(source['question'])) # NOTE: added this
        data = convert_huggingface_data_to_list_dic(dataset)
        data = process_truthful_qa(data)
    elif args.data == "cais/mmlu":
        dataset = load_dataset(args.data, "all", split="test")
        subsets = contamination.MMLU().subsets
        dataset = dataset.filter(lambda x: x['subject'] in subsets) # NOTE: added this
        if source is not None:
            dataset = dataset.filter(lambda x: x['question'] in list(source['question'])) # NOTE: added this
        data = convert_huggingface_data_to_list_dic(dataset)
